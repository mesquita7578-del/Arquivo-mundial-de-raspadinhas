...
import { X, Users, Clock, ShieldCheck, User, MapPin, Heart, Globe2 } from 'lucide-react';
        ...
        <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
           <div className="flex items-center gap-3">
              ...
              <div>
                 ...
                 <p className="text-slate-500 text-[8px] font-black uppercase tracking-widest">Radar de Acessos hihi!</p>
              </div>
           </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
           ...
           <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl text-center">
                 <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-0.5">Acessos Totais</span>
                 <span className="text-2xl font-black text-white italic tracking-tighter">{totalCount}</span>
              </div>
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl text-center">
                 <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-0.5">Em Tempo Real</span>
                 <span className="text-2xl font-black text-cyan-400 italic tracking-tighter animate-pulse">Ativo</span>
              </div>
           </div>

           <div className="space-y-2">
              <h3 className="text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-2 px-1">
                 <Clock className="w-2.5 h-2.5" /> Registos da Sessão
              </h3>
              ...
              {visitors.length === 0 ? (
                ...
                visitors.sort((a, b) => b.timestamp - a.timestamp).slice(0, 15).map((entry, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-slate-800/30 border border-slate-800 rounded-xl group hover:bg-slate-800 transition-all">
                    <div className="flex items-center gap-3">
                       <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${entry.isAdmin ? 'bg-amber-500 text-slate-950' : 'bg-brand-600 text-white'}`}>
                          {entry.isAdmin ? <ShieldCheck className="w-4 h-4" /> : <User className="w-4 h-4" />}
                       </div>
                       <div className="flex flex-col min-w-0">
                          <span className="text-xs font-black text-white uppercase tracking-tight truncate">{entry.name}</span>
                          <div className="flex items-center gap-1.5">
                             <MapPin className="w-2 h-2 text-cyan-500" />
                             <span className="text-[7px] font-black text-slate-500 uppercase truncate">{entry.location || 'Local Incerto'}</span>
                          </div>
                       </div>
                    </div>
           ...
           <div className="mt-6 bg-blue-900/10 border border-blue-500/10 p-4 rounded-2xl flex items-center gap-3">
              <Globe2 className="w-6 h-6 text-blue-500 shrink-0" />
              <p className="text-[9px] text-slate-400 italic leading-tight">
                 "O radar mostra-nos donde estão a ver o Arquivo Mundial em tempo real! Tudo seguro consigo vovô! hihi!"
              </p>
           </div>
        </div>
